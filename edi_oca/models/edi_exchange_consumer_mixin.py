...
class EDIExchangeConsumerMixin(models.AbstractModel):
    ...
    def _compute_exchange_record_ids(self):
        for rec in self:
            rec.exchange_record_ids = rec.exchange_related_record_ids.exchange_record_id

    def _edi_get_exchange_type_config(self):
        #  move this machinery to the rule model
        rules = (
    ...
    def edi_create_exchange_record(self, exchange_type_id):
        ...
        if (
            ...
            #  here you can still have more than one backend per type.
